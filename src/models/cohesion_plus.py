...
class COHESION_Plus(GeneralRecommender):
    def __init__(self, config, dataset):
        super(COHESION_Plus, self).__init__(config, dataset)
    ...
    def setup_parameters(self, config, dataset):
        ...
        self.nlgcl_layers = self.n_layers
    ...
    def get_knn_adj_mat(self, mm_embeddings):
        ...
        # norm - ensure indices are within bounds
        indices = torch.clamp(indices, 0, adj_size[0] - 1)
    ...
    def neighbor_cl_loss(self, embeddings_list, user, pos_item, neg_item):
        ...
        # Ensure embeddings are on the same device as indices
        device = user.device
        
        ego_embedding_u, ego_embedding_i = torch.split(embeddings_list[0].to(device), [self.n_users, self.n_items])
        cl_u = torch.tensor(0.0, device=device)
        cl_i = torch.tensor(0.0, device=device)
        
        for layer_idx in range(1, len(embeddings_list)):
            cur_embedding_u, cur_embedding_i = torch.split(embeddings_list[layer_idx].to(device), [self.n_users, self.n_items])
            
            # Ensure indices are within bounds and on correct device
            user_indices = torch.clamp(user, 0, self.n_users - 1).to(device)
            pos_item_indices = torch.clamp(pos_item, 0, self.n_items - 1).to(device)
            
            # Get embeddings with safe indexing
            cur_item_emb = cur_embedding_i[pos_item_indices]
            ego_user_emb = ego_embedding_u[user_indices]
            cur_user_emb = cur_embedding_u[user_indices]
            ego_item_emb = ego_embedding_i[pos_item_indices]
            
            cl_u = cl_u + self.InfoNCE(cur_item_emb, ego_user_emb, ego_user_emb) + 1e-6
            cl_i = cl_i + self.InfoNCE(cur_user_emb, ego_item_emb, ego_item_emb) + 1e-6
    ...
    def calculate_loss(self, interaction):
        ...
        id_embeddings_list = self.id_gcn.get_embeddings_list(self.id_feat, self.id_feat, self.masked_adj)
        v_embeddings_list = self.v_gcn.get_embeddings_list(self.v_feat, self.id_feat, self.masked_adj) if self.v_feat is not None else None
        t_embeddings_list = self.t_gcn.get_embeddings_list(self.t_feat, self.id_feat, self.masked_adj) if self.t_feat is not None else None
        ...
        if v_embeddings_list is not None and t_embeddings_list is not None:
            ...
            embeddings_list = []
            for idx in range(len(id_embeddings_list)):
                combined = (id_embeddings_list[idx] + v_embeddings_list[idx] + t_embeddings_list[idx]) / 3
                embeddings_list.append(combined)
        elif v_embeddings_list is not None:
            embeddings_list = [(id + v) / 2 for id, v in zip(id_embeddings_list, v_embeddings_list)]
        elif t_embeddings_list is not None:
            embeddings_list = [(id + t) / 2 for id, t in zip(id_embeddings_list, t_embeddings_list)]
        else:
            embeddings_list = id_embeddings_list
...
class GCNLayer(torch.nn.Module):
    ...
    def get_embeddings_list(self, features, id_embd, adj):
