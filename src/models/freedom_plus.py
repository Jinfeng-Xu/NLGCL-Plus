...
r"""
FREEDOM+NLGCL: Freezing and Denoising Graph Structures with Neighbor-aware Graph Contrastive Learning
Update: 01/08/2022 + NLGCL extension
"""
...
class FREEDOM_Plus(GeneralRecommender):
    def __init__(self, config, dataset):
        super(FREEDOM_Plus, self).__init__(config, dataset)
    ...
    def forward(self, adj):
        ...
        item_embeddings_list = [h]
        for i in range(self.n_layers):
            h = torch.sparse.mm(self.mm_adj, h)
            item_embeddings_list.append(h)
        ...
        # Stack for contrastive learning
        self.ui_embeddings_list = all_embeddings
    ...
    def neighbor_cl_loss(self, ui_embeddings_list, user, pos_item, neg_item):
        ...
        ego_embedding_u, ego_embedding_i = torch.split(ui_embeddings_list[0], [self.n_users, self.n_items])
        cl_u = 0
        cl_i = 0
        for layer_idx in range(1, len(ui_embeddings_list)):
            cur_embedding_u, cur_embedding_i = torch.split(ui_embeddings_list[layer_idx], [self.n_users, self.n_items])
    ...
    def calculate_loss(self, interaction):
        ...
        ego_cl_loss_u, ego_cl_loss_i = self.neighbor_cl_loss(self.ui_embeddings_list, users, pos_items, neg_items)
        ego_cl_loss = self.alpha * ego_cl_loss_u + (1 - self.alpha) * ego_cl_loss_i
        cl_loss = ego_cl_loss * self.cl_reg
        
        return batch_mf_loss + self.reg_weight * (mf_t_loss + mf_v_loss) + cl_loss
