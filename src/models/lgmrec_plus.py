...
r"""
LGMRec+NLGCL: Local and Global Graph Learning with Enhanced Neighbor-aware Contrastive Learning
AAAI'2024 + NLGCL extension
"""
...
class LGMRec_Plus(GeneralRecommender):
    def __init__(self, config, dataset):
        super(LGMRec_Plus, self).__init__(config, dataset)
        ...
        # Enhanced NLGCL parameters
        self.nlgcl_temp = config['nlgcl_temp']
        self.nlgcl_reg = config['nlgcl_reg']
        self.nlgcl_alpha = config['nlgcl_alpha']
        self.n_layers = self.n_ui_layers
        ...
        # HGNN layer for global hypergraph embedding
    ...
    def calculate_loss(self, interaction):
        ...
        # Enhanced NLGCL neighbor contrastive loss
        # Get LightGCN embeddings for neighbor CL
        ego_embeddings = torch.cat((self.user_embedding.weight, self.item_id_embedding.weight), dim=0)
        nlgcl_embeddings_list = [ego_embeddings]
        for _ in range(self.n_layers):
            ego_embeddings = torch.sparse.mm(self.norm_adj, ego_embeddings)
            nlgcl_embeddings_list.append(ego_embeddings)
        
        nlgcl_cl_u, nlgcl_cl_i = self.neighbor_cl_loss(nlgcl_embeddings_list, users, pos_items, neg_items)
        nlgcl_cl_loss = self.nlgcl_alpha * nlgcl_cl_u + (1 - self.nlgcl_alpha) * nlgcl_cl_i

        loss = batch_bpr_loss + self.cl_weight * batch_hcl_loss + self.reg_weight * batch_reg_loss + self.nlgcl_reg * nlgcl_cl_loss
